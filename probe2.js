...
(async()=>{
  ...
  await sleep(400);
  const bSpawn = B.m.find(x=>x.t==='respawn').p;
  const aSpawn = A.m.find(x=>x.t==='respawn').p;
  console.log('A spawn', aSpawn, 'B spawn', bSpawn);
  A.send({t:'input',p:bSpawn,q:[0,0,0,1],v:[0,0,0],th:0.5,h:100});
  await sleep(150);
  A.send({t:'fire',slot:0,w:'cannon',tgt:bSpawn,n:[0,0,-1],s:bSpawn});
  await sleep(500);
  console.log('--- B messages after fire:');
  for(const m of B.m) console.log(' ',m.t, JSON.stringify(m).slice(0,130));
